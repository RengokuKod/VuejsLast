<template>
    <button @click="addItem" class="new-button">Добавить нового посетителя</button>
    <div id="employers">
    <table>
    <tr>
    <th>id</th>
    ...
    <td>{{ item.Отчество }}</td>
    <td>{{ item.Возраст }}</td>
    <td>{{ item.Сумма_заказа }}</td>
    <td>{{ item.Количество_товаров }}</td>
    <td>{{ item.Тип_заказа  }}</td>
    <td>{{ item.Материал }}</td>
...
<button @click="deleteItem(item.id)" class="del-button">Удалить</button>
 ...
 methods: {
 editItem(item) {
    this.$router.push(`/edititem/${item.id}`)
 ...
 axios
 .delete(`http://77.222.52.42:8001/Posetitel/index/${id}`)
 .then(() => {
 ...
 mounted() {
 axios
 .get('http://77.222.52.42:8001/Posetitel/index')
 .then((response) => {
